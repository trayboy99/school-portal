...
-- Create student_exams table with CORRECT scoring logic
CREATE TABLE IF NOT EXISTS student_exams (
    ...
    -- For MIDTERM: CA1(0-10), CA2(0-10), Exam(0-20) = Total(40)
    -- For TERMINAL: CA1(0-20), CA2(0-20), Exam(0-60) = Total(100)
    ca1 INTEGER CHECK (
        (ca1 >= 0 AND ca1 <= 10 AND EXISTS(SELECT 1 FROM exams WHERE id = exam_id AND mark_type = 'midterm')) OR
        (ca1 >= 0 AND ca1 <= 20 AND EXISTS(SELECT 1 FROM exams WHERE id = exam_id AND mark_type = 'terminal'))
    ),
    ca2 INTEGER CHECK (
        (ca2 >= 0 AND ca2 <= 10 AND EXISTS(SELECT 1 FROM exams WHERE id = exam_id AND mark_type = 'midterm')) OR
        (ca2 >= 0 AND ca2 <= 20 AND EXISTS(SELECT 1 FROM exams WHERE id = exam_id AND mark_type = 'terminal'))
    ),
    exam INTEGER CHECK (
        (exam >= 0 AND exam <= 20 AND EXISTS(SELECT 1 FROM exams WHERE id = exam_id AND mark_type = 'midterm')) OR
        (exam >= 0 AND exam <= 60 AND EXISTS(SELECT 1 FROM exams WHERE id = exam_id AND mark_type = 'terminal'))
    ),
...
-- Function to calculate grades based on mark_type
...
-- CORRECT Function to copy midterm scores to terminal
-- Midterm CA1 + CA2 → Terminal CA1
-- Midterm Exam → Terminal CA2
-- Terminal Exam → Left empty
