...
-- Create broadsheet view using CORRECT column names from students table
CREATE OR REPLACE VIEW broadsheet_data AS
SELECT 
    s.id as student_id,
    s.first_name,
    s.surname,  -- Using 'surname' instead of 'last_name'
    s.class,
    e.exam_name,
    e.year,
    e.term,
    ...
    t.surname as teacher_surname,  -- Using 'surname' instead of 'last_name'
    se.ca1,
    se.ca2,
    se.exam,
    se.total,
...
ORDER BY s.class, s.surname, s.first_name, subj.subject_name;
