...
-- Show specific examples of grade changes (using correct column names)
SELECT 
...
-- Show the specific record you mentioned (33/40 score)
SELECT 
    'Your Specific Record (33/40)' as status,
    se.id,
    s.first_name as student_name,
    sub.subject_name,
    e.exam_name,
...
JOIN subjects sub ON se.subject_id = sub.id
JOIN exams e ON se.exam_id = e.id
WHERE se.total = 33 AND e.mark_type = 'midterm';
